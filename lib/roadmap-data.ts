...
// B站视频资源示例
const B_LINUX_BASIC: ResourceLink = { title: "【黑马】Linux零基础入门教程", url: "https://www.bilibili.com/video/BV1Q34y1D73U", required: false, type: "video", source: "bilibili", duration: "12h" };
const B_LINUX_CMD: ResourceLink = { title: "Linux常用命令速成", url: "https://www.bilibili.com/video/BV1bW411n7fY", required: false, type: "video", source: "bilibili", duration: "45min" };
...
const B_DOCKER_TUTORIAL: ResourceLink = { title: "【狂神说】Docker最新教程", url: "https://www.bilibili.com/video/BV1og4y1n7Dg", required: false, type: "video", source: "bilibili", duration: "3h" };
const B_PYTORCH_TUTORIAL: ResourceLink = { title: "【小土堆】PyTorch入门教程", url: "https://www.bilibili.com/video/BV1hE411t7Ly", required: false, type: "video", source: "bilibili", duration: "8h" };
...
export const FULL_ROADMAP: RoadmapNodeType[] = [
  ...
  // =====================================================
  {
    ...
    dailyTasks: [
      { day: 1, title: "仓库初始化与基础配置", content: ["Git 是分布式版本控制系统，它会记录代码的每一次修改，让你可以随时回退到任意历史版本、对比差异、多人协作。开始前需要用 git config 设置全局用户名和邮箱，这些信息会写入每次提交记录中", "git init 命令会在当前目录创建一个 .git 隐藏目录，里面存储所有的版本信息、配置、对象数据库和引用指针，有了它这个目录就变成了一个 Git 仓库", "git add 命令把工作区的修改加入暂存区（staging area），相当于告诉 Git '这些改动我要提交'；git commit 则把暂存区的内容生成一个永久快照，提交信息要写清楚这次改了什么、为什么改", "git log 命令查看完整的提交历史，包括作者、时间、提交信息；加上 --oneline 参数可以把每条提交压缩成一行，方便快速浏览"], duration: "1.5小时", resources: [R_GIT_SCM, { title: "GitHub Skills: Introduction to GitHub", url: "https://skills.github.com/", required: false }, { title: "廖雪峰 Git 教程", url: "https://liaoxuefeng.com/books/git/introduction/index.html", required: false }, { title: "【B站】Git从入门到精通", url: "https://www.bilibili.com/video/BV1vy4y1s7k6", required: false, type: "video", source: "bilibili", duration: "6h" }, { title: "awesome-git 资源列表", url: "https://github.com/dictcp/awesome-git", required: false, type: "repo", source: "github" }], checkpoint: "能从零创建 Git 仓库，完成一次 add + commit" },
      { day: 2, title: "理解 .git 目录与对象模型", content: ["Git 的所有数据都存储在 .git 目录中，用 ls -la .git/ 可以看到 HEAD 文件（指向当前分支）、objects 目录（存储所有数据对象）、refs 目录（存储分支和标签指针）、config 文件（仓库级配置）", "HEAD 文件记录了当前检出的分支引用，用 cat .git/HEAD 可以看到类似 ref: refs/heads/main 的内容；git rev-parse HEAD 命令可以把这个引用解析成完整的 40 位 SHA-1 哈希值", "Git 内部只有三种核心对象：blob 存储文件内容，tree 存储目录结构（类似文件夹），commit 存储一次提交的快照（包含 tree、作者、时间、父提交）。用 git cat-file -p <sha> 可以查看任意对象的内容", "可以用 echo 'hello' | git hash-object --stdin -w 手动创建一个 blob 对象，理解 Git 底层是如何通过哈希来存储和寻址数据的"], duration: "1.5小时", resources: [R_GIT_SCM, { title: "Git 内部原理 - 对象", url: "https://git-scm.com/book/zh/v2/Git-%E5%86%85%E9%83%A8%E5%8E%9F%E7%90%86-Git-%E5%AF%B9%E8%B1%A1", required: true }, { title: "Git Internals PDF by Scott Chacon", url: "https://github.com/pluralsight/git-internals-pdf", required: false }, { title: "【B站】Git内部原理深入理解", url: "https://www.bilibili.com/video/BV1vy4y1s7k6", required: false, type: "video", source: "bilibili", duration: "2h" }, { title: "git-vuln-model - Git安全漏洞模型", url: "https://github.com/so-called-pentesters/git-vuln-model", required: false, type: "repo", source: "github" }], checkpoint: "能解释 commit/tree/blob 三类对象的关系，并在 .git 中找到它们" },
      { day: 3, title: "分支创建、切换与合并", content: ["分支是 Git 最强大的功能之一，它本质上只是一个指向某次提交的轻量级指针。git checkout -b 命令会创建一个新分支并立即切换过去，让你在不影响主线（main）的情况下独立开发新功能或修复 bug", "在新分支上进行开发和提交，用 git log --graph --oneline 命令可以直观地看到分支的分叉和合并历史，理解 Git 的提交图结构", "开发完成后用 git merge 命令把分支合并回主线。如果两个分支修改了同一个文件的同一行，Git 无法自动合并，就会产生冲突，需要你手动决定保留哪些内容", "冲突解决的方法是：打开冲突文件，找到被 <<< 和 >>> 包裹的冲突标记，编辑保留正确的内容并删除所有标记，然后 git add 标记为已解决并提交"], duration: "2小时", resources: [R_GIT_BRANCHING, R_GIT_SCM, { title: "Atlassian Git 分支教程", url: "https://www.atlassian.com/git/tutorials/using-branches", required: false }, { title: "【B站】Git分支与合并实战", url: "https://www.bilibili.com/video/BV1vy4y1s7k6", required: false, type: "video", source: "bilibili", duration: "1.5h" }, { title: "git-flight-rules - Git故障排除指南", url: "https://github.com/k88hudson/git-flight-rules", required: false, type: "repo", source: "github" }], checkpoint: "能手动制造冲突并成功合并，git log 看到合并图" },
      { day: 4, title: "远程仓库与 push/pull", content: ["远程仓库是托管在服务器上的 Git 仓库副本，最常用的平台是 GitHub。先在 GitHub 上创建一个空仓库，然后用 git remote add origin 命令把本地仓库和远程仓库关联起来，origin 是远程仓库的默认别名", "git branch -M main 确保本地主分支名为 main（与 GitHub 默认一致），git push -u origin main 把本地的 main 分支推送到远程，-u 参数会建立上游跟踪关系，以后直接 git push 就行", "在 GitHub 网页上直接编辑 README.md 文件模拟'别人改了代码'的场景，然后用 git pull origin main 把远程的修改拉取到本地并自动合并，理解团队协作的基本流程", "git fetch origin 只下载远程的新提交但不合并，git pull 则是 fetch + merge 的组合。用 git remote -v 可以查看所有远程仓库的地址，理解 fetch 和 pull 的区别能避免很多协作中的困惑"], duration: "2小时", resources: [R_GIT_SCM, { title: "GitHub Docs: 连接到 GitHub", url: "https://docs.github.com/cn/get-started/getting-started-with-git/set-up-git", required: true }, { title: "Atlassian Git 远程仓库教程", url: "https://www.atlassian.com/git/tutorials/syncing", required: false }, { title: "【B站】Git远程仓库与协作开发", url: "https://www.bilibili.com/video/BV1vy4y1s7k6", required: false, type: "video", source: "bilibili", duration: "1h" }, { title: "git-remote-manager - Git远程仓库管理工具", url: "https://github.com/kimsangyeob/git-remote-manager", required: false, type: "repo", source: "github" }], checkpoint: "本地提交能成功推送到 GitHub，且能把远程修改 pull 回本地" },
      { day: 5, title: "Fork / 分支 / PR 流程", content: ["给开源项目贡献代码的标准流程是：先 Fork 把项目克隆到自己的 GitHub 账号下，然后在本地开发，最后发 Pull Request（PR）请求原作者把你的修改合并进去", "Fork 之后用 git clone 把自己账号下的仓库克隆到本地，再用 git remote add upstream 添加原始仓库作为上游远程，这样可以通过 git fetch upstream 保持与原项目的同步", "在本地创建 feature 分支进行开发，开发完成并测试通过后，push 到自己 fork 的远程仓库（origin），注意不要直接推到 main 分支", "在 GitHub 上点 New Pull Request 按钮，选择从你的 feature 分支合并到原项目的 main 分支，PR 描述要写清楚改了什么、为什么改、怎么测试的，然后请求原作者 review"], duration: "2小时", resources: [R_GIT_BRANCHING, R_GIT_SCM, { title: "GitHub Docs: Fork 项目", url: "https://docs.github.com/cn/pull-requests/collaborating-with-pull-requests/working-with-forks/fork-a-repo", required: true }, { title: "如何为开源项目做贡献（中文指南）", url: "https://opensource.guide/zh-hans/how-to-contribute/", required: false }, { title: "【B站】GitHub PR与开源协作流程", url: "https://www.bilibili.com/video/BV1vy4y1s7k6", required: false, type: "video", source: "bilibili", duration: "1h" }, { title: "first-contributions - 开源第一次贡献教程", url: "https://github.com/firstcontributions/first-contributions", required: false, type: "repo", source: "github" }], checkpoint: "能在自己的 GitHub 上发出人生第一个 Pull Request" },
    ...
    dailyTasks: [
      {
        ...
        resources: [
          ...
          { title: "【实例分割】COCO转YOLO格式转换", url: "https://www.bilibili.com/video/BV1qS4y1T7Cb", required: false, type: "video", source: "bilibili" },
      ...
      },
      {
        ...
        resources: [
          ...
          { title: "【YOLOv8】实例分割训练教程", url: "https://www.bilibili.com/video/BV1qS4y1T7Cb", required: false, type: "video", source: "bilibili" },
      ...
      },
      {
        ...
        resources: [
          ...
          { title: "【深度学习】迁移学习详解", url: "https://www.bilibili.com/video/BV1qS4y1T7Cb", required: false, type: "video", source: "bilibili" },
      ...
      },
      {
        ...
        resources: [
          ...
          { title: "【实例分割】推理与掩码渲染", url: "https://www.bilibili.com/video/BV1qS4y1T7Cb", required: false, type: "video", source: "bilibili" },
      ...
      },
      {
        ...
        resources: [
          ...
          { title: "【实例分割】端到端Pipeline实战", url: "https://www.bilibili.com/video/BV1qS4y1T7Cb", required: false, type: "video", source: "bilibili" },
  ...
  // =====================================================
  {
    ...
    dailyTasks: [
      { day: 1, title: "硬件抽象层概述", content: ["HAL（Hardware Abstraction Layer）是硬件和软件之间的抽象层，隐藏硬件差异", "HAL的作用：统一接口、跨平台移植、降低耦合、提高代码复用", "HAL层次结构：应用层→HAL层→驱动层→硬件层", "STM32 HAL库是典型的HAL实现，提供统一的API操作不同系列MCU"], duration: "1.5小时", resources: [{ title: "HAL概述", url: "https://www.st.com/resource/en/user_manual/dm00113874.pdf", required: true, type: "doc", source: "official" }, { title: "HAL设计原则", url: "https://www.state-machine.com/", required: false, type: "doc", source: "official" }, { title: "STM32 HAL教程", url: "https://www.bilibili.com/video/BV1th4y1X7Z9", required: false, type: "video", source: "bilibili" }, { title: "HAL框架源码", url: "https://github.com/STMicroelectronics/STM32CubeH7", required: false, type: "repo", source: "github" }, { title: "嵌入式HAL设计", url: "https://www.cnblogs.com/state-machine/category/HAL.html", required: false, type: "doc", source: "blog" }], checkpoint: "能解释HAL的作用和层次结构" },
      { day: 2, title: "HAL接口设计", content: ["HAL接口定义：函数声明、参数类型、返回值、错误码", "接口命名规范：HAL_外设_操作，如HAL_GPIO_WritePin、HAL_UART_Transmit", "参数设计：使用结构体配置参数，避免过多函数参数", "错误处理：返回HAL_StatusTypeDef（HAL_OK、HAL_ERROR、HAL_BUSY、HAL_TIMEOUT）"], duration: "2小时", resources: [{ title: "STM32 HAL", url: "https://www.st.com/resource/en/user_manual/dm00113874.pdf", required: true, type: "doc", source: "official" }, { title: "接口设计", url: "https://www.state-machine.com/", required: false, type: "doc", source: "official" }, { title: "HAL API设计模式", url: "https://www.bilibili.com/video/BV1J54y1P7qZ", required: false, type: "video", source: "bilibili" }, { title: "嵌入式接口设计", url: "https://github.com/embedded/hal-interface", required: false, type: "repo", source: "github" }, { title: "HAL驱动框架", url: "https://www.cnblogs.com/linux-embedded/p/hal-driver-framework.html", required: false, type: "doc", source: "blog" }], checkpoint: "能设计一个简单的HAL接口定义" },
      { day: 3, title: "BSP板级支持包", content: ["BSP（Board Support Package）是特定硬件板的初始化和配置代码", "BSP内容：时钟配置、GPIO初始化、外设初始化、引脚映射", "BSP与HAL的关系：BSP调用HAL初始化硬件，应用层调用HAL操作硬件", "BSP设计原则：模块化、可配置、清晰的初始化流程"], duration: "2小时", resources: [{ title: "BSP设计", url: "https://www.st.com/resource/en/user_manual/dm00113874.pdf", required: true, type: "doc", source: "official" }, { title: "STM32 BSP", url: "https://www.st.com/resource/en/user_manual/dm00113874.pdf", required: false, type: "doc", source: "official" }, { title: "STM32CubeMX BSP生成", url: "https://www.bilibili.com/video/BV1th4y1X7Z9", required: false, type: "video", source: "bilibili" }, { title: "STM32 BSP模板", url: "https://github.com/STMicroelectronics/STM32CubeH7", required: false, type: "repo", source: "github" }, { title: "BSP设计模式", url: "https://www.cnblogs.com/ARM-linux/p/BSP-design-patterns.html", required: false, type: "doc", source: "blog" }], checkpoint: "能编写一个简单板子的BSP初始化代码" },
      { day: 4, title: "跨平台移植技术", content: ["移植目标：同一份应用代码在不同硬件平台运行", "移植步骤：分析硬件差异→修改HAL实现→适配BSP→测试验证", "移植挑战：外设差异、时钟配置、引脚映射、中断处理", "移植最佳实践：最小化硬件相关代码、使用条件编译、编写移植指南"], duration: "2小时", resources: [{ title: "移植指南", url: "https://www.st.com/resource/en/user_manual/dm00113874.pdf", required: true, type: "doc", source: "official" }, { title: "跨平台开发", url: "https://www.state-machine.com/", required: false, type: "doc", source: "official" }, { title: "STM32跨平台移植", url: "https://www.bilibili.com/video/BV1Jf4y1R7xZ", required: false, type: "video", source: "bilibili" }, { title: "跨平台嵌入式框架", url: "https://github.com/embedded/cross-platform-hal", required: false, type: "repo", source: "github" }, { title: "嵌入式移植技术", url: "https://www.cnblogs.com/yuanf234/p/porting-guide.html", required: false, type: "doc", source: "blog" }], checkpoint: "能分析移植需要修改的代码部分" },
      { day: 5, title: "配置管理系统", content: ["配置文件：定义硬件参数、外设配置、功能开关", "配置方式：头文件宏定义、配置结构体、配置文件（JSON/YAML）", "配置工具：STM32CubeMX图形化配置工具，生成初始化代码", "配置管理最佳实践：集中管理、版本控制、配置验证"], duration: "2小时", resources: [{ title: "STM32CubeMX", url: "https://www.st.com/stm32cubemx", required: true, type: "tool", source: "official" }, { title: "配置管理", url: "https://www.state-machine.com/", required: false, type: "doc", source: "official" }, { title: "STM32CubeMX教程", url: "https://www.bilibili.com/video/BV1th4y1X7Z9", required: false, type: "video", source: "bilibili" }, { title: "Kconfig嵌入式配置", url: "https://github.com/embedded/kconfig-embedded", required: false, type: "repo", source: "github" }, { title: "嵌入式配置管理", url: "https://www.cnblogs.com/linux-embedded/p/kconfig-usage.html", required: false, type: "doc", source: "blog" }], checkpoint: "能用STM32CubeMX配置一个项目并生成代码" },
      { day: 6, title: "驱动框架设计", content: ["驱动框架：统一管理所有驱动，提供注册、初始化、操作接口", "驱动注册：驱动注册到框架，框架维护驱动列表", "驱动查找：通过驱动名称或ID查找驱动实例", "驱动框架示例：Linux设备驱动模型、FreeRTOS+框架"], duration: "2小时", resources: [{ title: "驱动框架", url: "https://www.kernel.org/doc/html/latest/driver-api/", required: true, type: "doc", source: "official" }, { title: "驱动模型", url: "https://www.state-machine.com/", required: false, type: "doc", source: "official" }, { title: "Linux驱动框架", url: "https://www.bilibili.com/video/BV1f94y1z7Np", required: false, type: "video", source: "bilibili" }, { title: "设备驱动框架", url: "https://github.com/linux-embedded/sensor-drivers", required: false, type: "repo", source: "github" }, { title: "驱动模型设计", url: "https://www.cnblogs.com/state-machine/p/driver-model.html", required: false, type: "doc", source: "blog" }], checkpoint: "能设计一个简单的驱动注册和查找框架" },
      { day: 7, title: "综合实践：设计一个HAL框架", content: ["综合项目：设计一个跨MCU的HAL框架，支持GPIO、UART、I2C、SPI", "框架结构：HAL接口层、HAL实现层、BSP层", "实现要求：定义统一接口、实现STM32版本、编写BSP", "测试框架在不同STM32系列上运行相同应用代码"], duration: "3小时", resources: [{ title: "HAL框架设计", url: "https://www.state-machine.com/", required: true, type: "doc", source: "official" }, { title: "STM32 HAL参考", url: "https://www.st.com/resource/en/user_manual/dm00113874.pdf", required: false, type: "doc", source: "official" }, { title: "HAL框架实战", url: "https://www.bilibili.com/video/BV1th4y1X7Z9", required: false, type: "video", source: "bilibili" }, { title: "跨平台HAL实现", url: "https://github.com/embedded/cross-platform-hal", required: false, type: "repo", source: "github" }, { title: "嵌入式框架设计", url: "https://www.cnblogs.com/yuanf234/p/hal-framework-design.html", required: false, type: "doc", source: "blog" }], checkpoint: "HAL框架能在不同MCU上运行，应用代码无需修改" }
    ]
  },
  ...
  // =====================================================
  {
    ...
    dailyTasks: [
      { day: 1, title: "PLC概述", content: ["PLC（Programmable Logic Controller）是工业自动化的核心控制器", "PLC组成：CPU、输入模块、输出模块、电源、通信接口", "PLC工作方式：循环扫描（读取输入→执行程序→更新输出→自诊断）", "PLC品牌：西门子S7系列、三菱FX系列、欧姆龙CP系列、AB（罗克韦尔）"], duration: "1.5小时", resources: [{ title: "PLC教程", url: "https://www.plcacademy.com/", required: true, type: "doc", source: "official" }, { title: "西门子PLC", url: "https://new.siemens.com/global/en/products/automation/systems/industrial/plc.html", required: false, type: "doc", source: "official" }, { title: "西门子S7-1200入门", url: "https://www.bilibili.com/video/BV1Q5411c7D1/", required: false, type: "video", source: "bilibili" }, { title: "PLC编程入门全套教程", url: "https://www.bilibili.com/video/BV1i7411c7Zp/", required: false, type: "video", source: "bilibili" }, { title: "西门子S7系列PLC专题", url: "https://github.com/topics/s7-plc", required: false, type: "repo", source: "github" }], checkpoint: "能解释PLC的组成和工作方式" },
      { day: 2, title: "PLC编程语言", content: ["IEC 61131-3标准定义了5种PLC编程语言", "梯形图（LD）：图形化语言，类似电气控制电路图，最常用", "功能块图（FBD）：图形化语言，用功能块连接表示逻辑", "结构化文本（ST）：文本语言，类似Pascal/C，适合复杂算法", "顺序功能图（SFC）：图形化语言，描述顺序控制流程"], duration: "2小时", resources: [{ title: "PLC编程语言", url: "https://www.plcacademy.com/plc-programming-languages/", required: true, type: "doc", source: "official" }, { title: "IEC 61131-3", url: "https://www.plcacademy.com/iec-61131-3/", required: false, type: "doc", source: "official" }, { title: "梯形图编程入门", url: "https://www.bilibili.com/video/BV1Ma4y1a7B8/", required: false, type: "video", source: "bilibili" }, { title: "三菱PLC编程入门", url: "https://www.bilibili.com/video/BV1uJ411k7H4/", required: false, type: "video", source: "bilibili" }, { title: "PLCopen标准资源", url: "https://www.plcopen.org/", required: false, type: "doc", source: "official" }], checkpoint: "能区分5种PLC编程语言的特点和适用场景" },
      { day: 3, title: "梯形图基础", content: ["梯形图结构：左右两条电源轨线，中间是逻辑元件和输出线圈", "基本元件：常开触点（X）、常闭触点（X NOT）、输出线圈（Y）", "逻辑组合：串联表示AND逻辑，并联表示OR逻辑", "梯形图执行：从左到右、从上到下顺序执行"], duration: "2.5小时", resources: [{ title: "梯形图教程", url: "https://www.plcacademy.com/ladder-logic/", required: true, type: "doc", source: "official" }, { title: "梯形图练习", url: "https://www.plc-fiddle.com/", required: false, type: "tool", source: "official" }, { title: "梯形图从入门到精通", url: "https://www.bilibili.com/video/BV1KJ411k7fU/", required: false, type: "video", source: "bilibili" }, { title: "西门子S7梯形图编程", url: "https://www.bilibili.com/video/BV1CJ411t7NY/", required: false, type: "video", source: "bilibili" }, { title: "OpenPLC开源项目", url: "https://github.com/thiagoralves/openplc", required: false, type: "repo", source: "github" }], checkpoint: "能用梯形图实现基本的逻辑控制" },
      { day: 4, title: "定时器与计数器", content: ["定时器（Timer）：延时控制，TON（通电延时）、TOF（断电延时）、TP（脉冲定时）", "定时器参数：预设值（PT）、当前值（ET）、输出（Q）", "计数器（Counter）：计数控制，CTU（加计数）、CTD（减计数）、CTUD（加减计数）", "计数器参数：预设值（PV）、当前值（CV）、输出（Q）"], duration: "2.5小时", resources: [{ title: "定时器计数器", url: "https://www.plcacademy.com/timers-and-counters/", required: true, type: "doc", source: "official" }, { title: "定时器练习", url: "https://www.plc-fiddle.com/", required: false, type: "tool", source: "official" }, { title: "定时器应用详解", url: "https://www.bilibili.com/video/BV1JE411k7hW/", required: false, type: "video", source: "bilibili" }, { title: "PLC计数器编程", url: "https://www.bilibili.com/video/BV1Mx41187qJ/", required: false, type: "video", source: "bilibili" }, { title: "工业控制论坛-工控人家", url: "https://www.gkrb88.com/", required: false, type: "doc", source: "official" }], checkpoint: "能用定时器和计数器实现延时和计数控制" },
      { day: 5, title: "数据处理与运算", content: ["数据类型：BOOL（布尔）、INT（整数）、REAL（浮点）、STRING（字符串）", "比较指令：等于、不等于、大于、小于、大于等于、小于等于", "算术指令：加、减、乘、除、取模", "数据移动：MOV指令复制数据，数据类型转换指令"], duration: "2小时", resources: [{ title: "数据处理", url: "https://www.plcacademy.com/plc-data-handling/", required: true, type: "doc", source: "official" }, { title: "算术运算", url: "https://www.plcacademy.com/plc-math-instructions/", required: false, type: "doc", source: "official" }, { title: "PLC数据处理教程", url: "https://www.bilibili.com/video/BV1Vx41187fP/", required: false, type: "video", source: "bilibili" }, { title: "S7-200 SMART数据运算", url: "https://www.bilibili.com/video/BV1CJ411t7NY/", required: false, type: "video", source: "bilibili" }, { title: "MATLAB PLC工具箱", url: "https://github.com/mbachmann/matlab-plc", required: false, type: "repo", source: "github" }], checkpoint: "能用数据处理指令实现数值比较和运算" },
      { day: 6, title: "模拟量处理", content: ["模拟量输入：温度、压力、流量等连续信号，通过ADC转换为数字值", "模拟量输出：控制变频器、调节阀等，通过DAC转换为模拟信号", "模拟量模块：AI模块（4-20mA、0-10V）、AO模块", "模拟量处理：量程转换、滤波、PID控制"], duration: "2.5小时", resources: [{ title: "模拟量处理", url: "https://www.plcacademy.com/analog-signals/", required: true, type: "doc", source: "official" }, { title: "PID控制", url: "https://www.plcacademy.com/plc-pid-control/", required: false, type: "doc", source: "official" }, { title: "模拟量编程实战", url: "https://www.bilibili.com/video/BV1ME411k7m5/", required: false, type: "video", source: "bilibili" }, { title: "PLC模拟量与PID调节", url: "https://www.bilibili.com/video/BV1QJ411k7qU/", required: false, type: "video", source: "bilibili" }, { title: "工控论坛模拟量专题", url: "https://bbs.hcbbs.com/", required: false, type: "doc", source: "official" }], checkpoint: "能配置模拟量模块并实现量程转换" },
      { day: 7, title: "PLC通信", content: ["PLC通信：PLC与PLC、PLC与上位机、PLC与现场设备之间的数据交换", "现场总线：Profibus、Modbus、DeviceNet、CANopen", "工业以太网：Profinet、EtherNet/IP、EtherCAT", "OPC UA：工业通信标准，支持跨平台数据交换"], duration: "2小时", resources: [{ title: "PLC通信", url: "https://www.plcacademy.com/plc-communication/", required: true, type: "doc", source: "official" }, { title: "Modbus", url: "https://www.modbus.org/", required: false, type: "doc", source: "official" }, { title: "Modbus通信协议详解", url: "https://www.bilibili.com/video/BV1MJ411k7bX/", required: false, type: "video", source: "bilibili" }, { title: "Profinet通信配置", url: "https://www.bilibili.com/video/BV1fJ411k7LH/", required: false, type: "video", source: "bilibili" }, { title: "libmodbus开源库", url: "https://github.com/stephane/libmodbus", required: false, type: "repo", source: "github" }], checkpoint: "能解释常见工业通信协议的特点" },
      { day: 8, title: "HMI人机界面", content: ["HMI（Human Machine Interface）是人机交互界面，用于监视和控制PLC", "HMI功能：显示状态、参数设置、报警提示、历史记录、趋势图", "HMI设计原则：界面简洁、操作方便、信息清晰、安全可靠", "HMI软件：西门子WinCC、三菱GT Designer、AB FactoryTalk"], duration: "2小时", resources: [{ title: "HMI教程", url: "https://www.plcacademy.com/hmi-human-machine-interface/", required: true, type: "doc", source: "official" }, { title: "HMI设计", url: "https://new.siemens.com/global/en/products/automation/software-engineering/hmi.html", required: false, type: "doc", source: "official" }, { title: "WinCC入门教程", url: "https://www.bilibili.com/video/BV1NJ411s7Mu/", required: false, type: "video", source: "bilibili" }, { title: "威纶通HMI编程", url: "https://www.bilibili.com/video/BV1ME411k7m5/", required: false, type: "video", source: "bilibili" }, { title: "FreeSCADA开源项目", url: "https://github.com/FreeSCADA", required: false, type: "repo", source: "github" }], checkpoint: "能设计一个简单的HMI界面" },
  ...
  // =====================================================
  {
    ...
    dailyTasks: [
      { day: 1, title: "伺服系统概述", content: ["伺服系统：精确控制位置、速度、加速度的闭环控制系统", "伺服系统组成：伺服电机、伺服驱动器、编码器、控制器", "伺服电机类型：交流伺服电机（PMSM）、直流伺服电机、步进电机", "伺服应用：数控机床、机器人关节、自动化设备、精密定位"], duration: "1.5小时", resources: [{ title: "伺服系统", url: "https://www.motioncontroltips.com/what-is-a-servo-system/", required: true, type: "doc", source: "official" }, { title: "伺服电机", url: "https://www.mitsubishielectric.com/fa/products/drv/servo/", required: false, type: "doc", source: "official" }, { title: "伺服系统入门", url: "https://www.bilibili.com/video/BV1Jh411Z7K4/", required: false, type: "video", source: "bilibili" }, { title: "伺服电机原理", url: "https://www.bilibili.com/video/BV1MJ411b7R5/", required: false, type: "video", source: "bilibili" }, { title: "ServoKit开源库", url: "https://github.com/adafruit/Adafruit_CircuitPython_ServoKit", required: false, type: "repo", source: "github" }], checkpoint: "能解释伺服系统的组成和应用场景" },
      { day: 2, title: "编码器与位置检测", content: ["编码器：检测电机位置和速度的传感器，是伺服系统的核心反馈元件", "编码器类型：增量式编码器（输出脉冲）、绝对式编码器（输出位置值）", "编码器分辨率：每转脉冲数（PPR），越高定位精度越高", "编码器信号：A/B/Z三相脉冲，A/B用于计数和方向判断，Z用于零位参考"], duration: "2.5小时", resources: [{ title: "编码器原理", url: "https://www.motioncontroltips.com/what-is-an-encoder/", required: true, type: "doc", source: "official" }, { title: "编码器选型", url: "https://www.heidenhain.com/", required: false, type: "doc", source: "official" }, { title: "编码器原理与应用", url: "https://www.bilibili.com/video/BV1ME411k7m5/", required: false, type: "video", source: "bilibili" }, { title: "PLC编码器编程", url: "https://www.bilibili.com/video/BV1Vx41187fP/", required: false, type: "video", source: "bilibili" }, { title: "Python编码器库", url: "https://github.com/simondlevy/RoboSat", required: false, type: "repo", source: "github" }], checkpoint: "能解释增量式和绝对式编码器的区别" },
      { day: 3, title: "伺服驱动器", content: ["伺服驱动器：控制伺服电机的功率放大器和控制器", "伺服驱动器功能：功率放大、电流控制、速度控制、位置控制、保护功能", "伺服驱动器参数：电子齿轮比、速度环增益、位置环增益、加减速时间", "伺服驱动器通信：脉冲控制、模拟量控制、总线控制（Modbus、EtherCAT）"], duration: "2.5小时", resources: [{ title: "伺服驱动器", url: "https://www.motioncontroltips.com/what-is-a-servo-drive/", required: true, type: "doc", source: "official" }, { title: "驱动器配置", url: "https://www.mitsubishielectric.com/fa/products/drv/servo/", required: false, type: "doc", source: "official" }, { title: "伺服驱动器调试", url: "https://www.bilibili.com/video/BV1ME411k7m5/", required: false, type: "video", source: "bilibili" }, { title: "松下驱动器配置", url: "https://www.bilibili.com/video/BV1Vx41187fP/", required: false, type: "video", source: "bilibili" }, { title: "EtherCAT开源栈", url: "https://github.com/OpenEtherCAT society/SOES", required: false, type: "repo", source: "github" }], checkpoint: "能配置伺服驱动器的基本参数" },
      { day: 4, title: "位置控制模式", content: ["位置控制：精确控制电机到达指定位置", "位置指令方式：脉冲指令（方向+脉冲）、总线指令（位置值）", "电子齿轮比：指令脉冲与实际移动距离的换算比例", "定位精度：取决于编码器分辨率、机械传动精度、控制算法"], duration: "2小时", resources: [{ title: "位置控制", url: "https://www.motioncontroltips.com/servo-position-control/", required: true, type: "doc", source: "official" }, { title: "电子齿轮", url: "https://www.mitsubishielectric.com/fa/products/drv/servo/", required: false, type: "doc", source: "official" }, { title: "伺服位置控制编程", url: "https://www.bilibili.com/video/BV1ME411k7m5/", required: false, type: "video", source: "bilibili" }, { title: "电子齿轮比计算", url: "https://www.bilibili.com/video/BV1Vx41187fP/", required: false, type: "video", source: "bilibili" }, { title: "运动控制Simulink模型", url: "https://github.com/mathworks/Simulink servo-control", required: false, type: "repo", source: "github" }], checkpoint: "能计算电子齿轮比实现指定定位精度" },
      { day: 5, title: "速度控制模式", content: ["速度控制：控制电机以指定速度运行", "速度指令方式：模拟电压（0-10V）、总线指令（速度值）", "速度环控制：PI控制器调节速度误差，输出电流指令", "速度波动：负载变化、摩擦、共振等因素影响速度稳定性"], duration: "2小时", resources: [{ title: "速度控制", url: "https://www.motioncontroltips.com/servo-speed-control/", required: true, type: "doc", source: "official" }, { title: "速度环调试", url: "https://www.mitsubishielectric.com/fa/products/drv/servo/", required: false, type: "doc", source: "official" }, { title: "伺服速度模式调试", url: "https://www.bilibili.com/video/BV1ME411k7m5/", required: false, type: "video", source: "bilibili" }, { title: "速度环PID调节", url: "https://www.bilibili.com/video/BV1Vx41187fP/", required: false, type: "video", source: "bilibili" }, { title: "MATLAB速度控制仿真", url: "https://github.com/mathworks/speed-control-servo", required: false, type: "repo", source: "github" }], checkpoint: "能调试速度环参数实现稳定速度控制" },
      { day: 6, title: "转矩控制模式", content: ["转矩控制：控制电机输出指定转矩", "转矩指令方式：模拟电压（0-10V）、总线指令（转矩值）", "转矩控制应用：张力控制、卷绕控制、压力控制、力控机器人", "转矩限制：设置最大转矩保护电机和机械系统"], duration: "2小时", resources: [{ title: "转矩控制", url: "https://www.motioncontroltips.com/servo-torque-control/", required: true, type: "doc", source: "official" }, { title: "转矩控制应用", url: "https://www.mitsubishielectric.com/fa/products/drv/servo/", required: false, type: "doc", source: "official" }, { title: "张力控制详解", url: "https://www.bilibili.com/video/BV1ME411k7m5/", required: false, type: "video", source: "bilibili" }, { title: "转矩模式应用", url: "https://www.bilibili.com/video/BV1Vx41187fP/", required: false, type: "video", source: "bilibili" }, { title: "力控机器人教程", url: "https://github.com/google/robotics-servo", required: false, type: "repo", source: "github" }], checkpoint: "能解释转矩控制的应用场景" },
      { day: 7, title: "运动规划", content: ["运动规划：生成从起点到终点的运动轨迹", "运动轨迹类型：梯形速度曲线、S形速度曲线、多项式曲线", "加减速控制：避免冲击和振动，提高运动平稳性", "多轴协调：多轴联动实现复杂轨迹，如直线插补、圆弧插补"], duration: "2.5小时", resources: [{ title: "运动规划", url: "https://www.motioncontroltips.com/motion-profiles/", required: true, type: "doc", source: "official" }, { title: "轨迹规划", url: "https://www.mathworks.com/help/robotics/ug/trajectory-planning.html", required: false, type: "doc", source: "official" }, { title: "S型曲线规划", url: "https://www.bilibili.com/video/BV1ME411k7m5/", required: false, type: "video", source: "bilibili" }, { title: "梯形速度曲线编程", url: "https://www.bilibili.com/video/BV1Vx41187fP/", required: false, type: "video", source: "bilibili" }, { title: "运动规划开源库", url: "https://github.com/machinaai/panster", required: false, type: "repo", source: "github" }], checkpoint: "能设计梯形和S形速度曲线" },
      { day: 8, title: "步进电机控制", content: ["步进电机：开环控制，每脉冲转动固定角度，成本低但精度有限", "步进电机类型：反应式步进、永磁式步进、混合式步进", "步距角：每脉冲转动的角度，常见1.8°（200步/转）或0.9°", "细分驱动：通过控制电流实现更小步距角，提高分辨率和平稳性"], duration: "2小时", resources: [{ title: "步进电机", url: "https://www.motioncontroltips.com/what-is-a-stepper-motor/", required: true, type: "doc", source: "official" }, { title: "步进驱动", url: "https://www.trinamic.com/", required: false, type: "doc", source: "official" }, { title: "步进电机编程", url: "https://www.bilibili.com/video/BV1ME411k7m5/", required: false, type: "video", source: "bilibili" }, { title: "细分驱动原理", url: "https://www.bilibili.com/video/BV1Vx41187fP/", required: false, type: "video", source: "bilibili" }, { title: "StepperMotor库", url: "https://github.com/arduino-libraries/Stepper", required: false, type: "repo", source: "github" }], checkpoint: "能解释步进电机的工作原理和细分驱动" },
      { day: 9, title: "伺服系统调试", content: ["伺服调试：调整控制参数使系统稳定、响应快、精度高", "调试步骤：先电流环→再速度环→最后位置环", "增益调整：增大增益提高响应速度，但过大会导致振动和超调", "惯量匹配：负载惯量与电机惯量比值影响控制性能，通常要求比值小于5"], duration: "2.5小时", resources: [{ title: "伺服调试", url: "https://www.motioncontroltips.com/servo-tuning/", required: true, type: "doc", source: "official" }, { title: "调试方法", url: "https://www.mitsubishielectric.com/fa/products/drv/servo/", required: false, type: "doc", source: "official" }, { title: "伺服调试教程", url: "https://www.bilibili.com/video/BV1ME411k7m5/", required: false, type: "video", source: "bilibili" }, { title: "增益调整实战", url: "https://www.bilibili.com/video/BV1Vx41187fP/", required: false, type: "video", source: "bilibili" }, { title: "伺服调参Simulink", url: "https://github.com/mathworks/servo-tuning", required: false, type: "repo", source: "github" }], checkpoint: "能调试伺服系统参数实现稳定控制" },
      { day: 10, title: "综合实践：数控定位系统", content: ["综合项目：设计一个数控定位系统，实现精密位置控制", "功能要求：位置指令输入→伺服控制→精密定位→位置反馈", "硬件配置：控制器、伺服驱动器、伺服电机、编码器、导轨丝杠", "软件实现：位置指令处理、运动规划、伺服控制、误差补偿"], duration: "3小时", resources: [{ title: "定位系统", url: "https://www.motioncontroltips.com/positioning-systems/", required: true, type: "doc", source: "official" }, { title: "数控系统", url: "https://www.fanuc.com/", required: false, type: "doc", source: "official" }, { title: "数控系统项目实战", url: "https://www.bilibili.com/video/BV1ME411k7m5/", required: false, type: "video", source: "bilibili" }, { title: "伺服定位项目", url: "https://www.bilibili.com/video/BV1Vx41187fP/", required: false, type: "video", source: "bilibili" }, { title: "数控系统开源项目", url: "https://github.com/LinuxCNC/linuxcnc", required: false, type: "repo", source: "github" }], checkpoint: "定位系统能实现精密定位，误差在允许范围内" }
    ]
  },
