...
export const FULL_ROADMAP: RoadmapNodeType[] = [
  ...
  // =====================================================
  {
    ...
    dailyTasks: [
      ...
      { day: 7, title: "git diff / stash / reset / checkout", content: ["git diff 命令可以对比不同区域之间的差异：不加参数对比工作区和暂存区，--cached 对比暂存区和最新提交，HEAD 对比工作区和最新提交，这是理解 Git 三层结构（工作区、暂存区、仓库）的关键", "git stash 命令可以把当前未提交的修改临时保存起来，让你能干净地切换到其他分支处理紧急事务。git stash push -m 'wip' 保存并命名，git stash list 查看列表，git stash pop 恢复最近一次并删除记录", "git reset 是撤销提交的核心命令：--soft 只撤销提交保留暂存区和工作区（适合重新提交），--mixed 撤销提交和暂存区保留工作区（默认模式），--hard 全部丢弃回到指定提交状态（危险操作）", "git checkout -- file 可以丢弃工作区中某个文件的修改（恢复到暂存区状态），git clean -fd 可以删除所有未被 Git 跟踪的文件和目录，这两个都是不可逆操作，使用前要确认"], duration: "1.5小时", resources: [B_GIT_TUTORIAL, R_GIT_BRANCHING, R_GIT_SCM, { title: "Oh My Git! 游戏化学 Git", url: "https://ohmygit.org/", required: false }], checkpoint: "能解释 soft/mixed/hard 三种 reset 的区别并各用一次" },
  ...
  // =====================================================
  {
    ...
    dailyTasks: [
      { day: 1, title: "LLM 生态与模型规模", content: ["HuggingFace 模型卡是了解模型的第一入口，上面展示了架构信息、训练细节、许可协议和社区评价。重点关注 mistralai/Mistral-7B-v0.1、TinyLlama/TinyLlama-1.1B-Chat-v1.0 等典型模型", "理解模型规模的关键维度：参数量决定了显存需求，训练 token 数影响模型能力上限，上下文长度决定了能处理的最长文本，许可协议决定了能否商用", "本地硬件评估：一张 24GB 显卡用 fp16 加载 7B 模型大约占用 13GB 显存；如果显存不足，可以通过 4-bit 或 8-bit 量化来降低显存占用，从而在消费级显卡上运行更大模型", "选择一个轻量模型做入门演示：TinyLlama 1.1B 是资源最友好的选择，适合快速验证流程，之后再切换到更大的模型"], duration: "1.5小时", resources: [B_LLM_FINETUNE, R_HF_TRANSFORMERS, { title: "HuggingFace Open LLM Leaderboard", url: "https://huggingface.co/spaces/open-llm-leaderboard/open_llm_leaderboard", required: false }, { title: "LLM 模型参数量与显存计算指南", url: "https://huggingface.co/docs/transformers/perf_train_gpu_one", required: false }], checkpoint: "能在 HuggingFace 上找到 ≤ 3B 的开源中文/英文模型，并成功 AutoModelForCausalLM.from_pretrained 加载" },
  ...
  // =====================================================
  {
    ...
    dailyTasks: [
      { day: 1, title: "通信系统基本模型", content: ["通信系统由信源、发送设备、信道、接收设备、信宿组成", "模拟通信和数字通信的区别：模拟信号连续取值，数字信号离散取值", "数字通信的优势：抗干扰能力强、易于加密、便于信号处理和存储、可利用纠错编码提高可靠性", "在AI物联网应用中，无线传感器节点采集数据后通过数字通信发送到云端服务器"], duration: "1.5小时", resources: [B_ML_LIHONGYI, { title: "通信原理教材", url: "https://www.g很低.com/comm/", required: false, type: "book", source: "other" }, { title: "MATLAB通信工具箱", url: "https://www.mathworks.com/products/communications.html", required: false, type: "tool", source: "official" }, { title: "通信系统仿真项目", url: "https://github.com/R欺bySandbox/CommunicationSystems", required: false, type: "repo", source: "github" }], checkpoint: "能画出数字通信系统的完整框图，说明各部分作用" },
  ...
  // =====================================================
  {
    ...
    dailyTasks: [
      { day: 1, title: "PCB概述", content: ["PCB（Printed Circuit Board）是电子元器件的载体，提供电气连接和机械支撑", "PCB类型：单面板、双面板、多层板（4层、6层等）", "PCB结构：基材（FR4）、铜箔层、阻焊层、丝印层、表面处理", "在AI硬件开发中，PCB是连接芯片、传感器、电源的基础"], duration: "1.5小时", resources: [B_ML_LIHONGYI, { title: "PCB基础", url: "https://www.allaboutcircuits.com/textbook/semiconductors/chpt-15/printed-circuit-board-pcb/", required: true, type: "doc", source: "official" }, { title: "PCB设计流程", url: "https://www.sparkfun.com/tutorials/115", required: false, type: "doc", source: "official" }], checkpoint: "能解释PCB的结构和各层的作用" },
  ...
  // =====================================================
  {
    ...
    dailyTasks: [
      { day: 1, title: "DSP概述", content: ["DSP（Digital Signal Processing）是用数字方法处理模拟信号的技术", "DSP系统组成：ADC采样→数字处理→DAC输出", "DSP优势：精度高、可编程、可复现、抗干扰", "DSP应用：音频处理、图像处理、通信、雷达、医疗仪器"], duration: "1.5小时", resources: [B_ML_LIHONGYI, { title: "DSP教程", url: "https://www.ni.com/zh-cn/innovations/white-papers/13/digital-signal-processing-fundamentals.html", required: true, type: "doc", source: "official" }, { title: "DSP工具", url: "https://www.mathworks.com/products/dsp-system.html", required: false, type: "tool", source: "official" }], checkpoint: "能解释DSP系统的组成和优势" },
