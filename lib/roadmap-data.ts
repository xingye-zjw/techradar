...
const B_LINEAR_ALGEBRA: ResourceLink = { title: "【3Blue1Brown】线性代数的本质", url: "https://www.bilibili.com/video/BV1ys4114C2W", required: false, type: "video", source: "bilibili", duration: "3h" };
const B_PROBABILITY: ResourceLink = { title: "【可汗学院】概率与统计", url: "https://www.bilibili.com/video/BV1WV411W7pP", required: false, type: "video", source: "bilibili", duration: "4h" };
...
const B_NLP_TUTORIAL: ResourceLink = { title: "【李宏毅】NLP自然语言处理", url: "https://www.bilibili.com/video/BV1Wv411Q7pN", required: false, type: "video", source: "bilibili", duration: "8h" };
const B_TRANSFORMER_TUTORIAL: ResourceLink = { title: "【李宏毅】Transformer详解", url: "https://www.bilibili.com/video/BV1Mv411M7pT", required: false, type: "video", source: "bilibili", duration: "4h" };
const B_LLM_TUTORIAL: ResourceLink = { title: "【李宏毅】大模型微调实战", url: "https://www.bilibili.com/video/BV1KV411Q7pN", required: false, type: "video", source: "bilibili", duration: "5h" };
const B_RAG_TUTORIAL: ResourceLink = { title: "【RAG检索增强生成】实战教程", url: "https://www.bilibili.com/video/BV1Vz4y1R7FG", required: false, type: "video", source: "bilibili", duration: "3h" };
...
export const FULL_ROADMAP: RoadmapNodeType[] = [
  ...
  // =====================================================
  {
    ...
    dailyTasks: [
      { day: 1, title: "卷积操作与感受野", content: ["卷积操作是 CNN 的核心，通过在输入图像上滑动一个小的卷积核（filter），逐位置计算点积来提取特征。可以手动构造一个 Sobel 边缘检测核，用 torch.nn.functional.conv2d 对灰度图做卷积，观察响应图中边缘被高亮", "padding 是在输入边缘填充零值，可以控制输出尺寸。padding=1 在 3x3 卷积核下保持空间尺寸不变（same padding）。没有 padding 时输出会比输入小（valid convolution）", "输出尺寸的计算公式为 H' = (H + 2P - K) // S + 1，其中 H 是输入高度，P 是 padding，K 是核大小，S 是步长。这个公式是设计网络架构时必须掌握的基础", "感受野（Receptive Field）是指输出特征图上一个像素对应原始输入的区域大小。堆叠多层小卷积核（如 3x3）可以在保持较少参数的同时获得更大的感受野，这是 VGG 网络的核心思想"], duration: "2小时", resources: [R_CS231N, R_D2L, B_CV_TUTORIAL, { title: "卷积神经网络可视化讲解", url: "https://cs231n.github.io/convolutional-networks/", required: true }, { title: "卷积动画演示", url: "https://github.com/vdumoulin/conv_arithmetic", required: false }, { title: "【计算机视觉】卷积神经网络入门", url: "https://www.bilibili.com/video/BV1S64y1T7zi", required: false, type: "video", source: "bilibili" }, { title: "卷积操作PyTorch实现", url: "https://pytorch.org/docs/stable/generated/torch.nn.Conv2d.html", required: false, type: "doc", source: "official" }], checkpoint: "能手工计算给定输入 shape、kernel、stride、padding 时的输出 shape" },
  ...
  // =====================================================
  {
    ...
    dailyTasks: [
      { day: 1, title: "LLM 生态与模型规模", content: ["HuggingFace 模型卡是了解模型的第一入口，上面展示了架构信息、训练细节、许可协议和社区评价。重点关注 mistralai/Mistral-7B-v0.1、TinyLlama/TinyLlama-1.1B-Chat-v1.0 等典型模型", "理解模型规模的关键维度：参数量决定了显存需求，训练 token 数影响模型能力上限，上下文长度决定了能处理的最长文本，许可协议决定了能否商用", "本地硬件评估：一张 24GB 显卡用 fp16 加载 7B 模型大约占用 13GB 显存；如果显存不足，可以通过 4-bit 或 8-bit 量化来降低显存占用，从而在消费级显卡上运行更大模型", "选择一个轻量模型做入门演示：TinyLlama 1.1B 是资源最友好的选择，适合快速验证流程，之后再切换到更大的模型"], duration: "1.5小时", resources: [R_HF_TRANSFORMERS, B_LLM_TUTORIAL, { title: "HuggingFace Open LLM Leaderboard", url: "https://huggingface.co/spaces/open-llm-leaderboard/open_llm_leaderboard", required: false }, { title: "LLM 模型参数量与显存计算指南", url: "https://huggingface.co/docs/transformers/perf_train_gpu_one", required: false }], checkpoint: "能在 HuggingFace 上找到 ≤ 3B 的开源中文/英文模型，并成功 AutoModelForCausalLM.from_pretrained 加载" },
  ...
  // =====================================================
  {
    ...
    dailyTasks: [
      ...
      { day: 8, title: "中断处理与DMA", content: ["中断是硬件通知CPU的机制，CPU暂停当前任务执行中断服务程序（ISR）", "中断向量表：存储各中断源的ISR入口地址，CPU根据中断号跳转执行", "NVIC（嵌套向量中断控制器）：Cortex-M的中断管理器，支持优先级和嵌套", "DMA（直接内存访问）：外设直接读写内存，无需CPU参与，提高数据传输效率"], duration: "2.5小时", resources: [{ title: "中断处理", url: "https://www.st.com/resource/en/application_note/dm00042534.pdf", required: true, type: "doc", source: "official" }, { title: "DMA教程", url: "https://www.st.com/resource/en/application_note/dm00042534.pdf", required: false, type: "doc", source: "official" }, { title: "Cortex-M中断系统", url: "https://www.bilibili.com/video/BV1V54y1P7V1", required: false, type: "video", source: "bilibili" },  { title: "DMA驱动仓库", url: "https://github.com/MaJerle/stm32-dma", required: false, type: "repo", source: "github" }], checkpoint: "能编写中断处理程序和配置DMA传输" },
