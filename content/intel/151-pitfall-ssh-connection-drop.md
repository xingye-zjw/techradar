...
1.  客户端编辑 ~/.ssh/config 添加：ServerAliveInterval 60 + ServerAliveCountMax 3
2.  服务器端编辑 /etc/ssh/sshd_config：ClientAliveInterval 60
3.  使用 tmux 或 screen：SSH 断开后会话保持运行
4.  传输大文件使用 rsync -avz --progress 并加 screen 后台运行
5.  长期训练任务用 nohup 或 systemd service 替代 SSH 会话
