...
1.  Docker run 时加 -v /etc/localtime:/etc/localtime:ro 挂载宿主机时区
2.  或 Docker run 加 -e TZ=Asia/Shanghai 设置容器环境变量
3.  在 Dockerfile 中 RUN ln -snf /usr/share/zoneinfo/Asia/Shanghai /etc/localtime
4.  定时任务用 cron 时确保容器内时区正确：ENV TZ=Asia/Shanghai
5.  日志框架（Python logging / loguru）确认使用了正确的时区配置
