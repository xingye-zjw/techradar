...
01. 使用Valgrind检测内存错误：valgrind --leak-check=full ./program
02. 数组访问前检查索引范围：if (index >= 0 && index < array_size)
03. 字符串操作使用安全版本：strncpy替代strcpy，snprintf替代sprintf
04. 指针释放后设为NULL：free(ptr); ptr = NULL;
05. 嵌入式开发中使用MPU（内存保护单元）限制访问范围
06. 启用编译器边界检查选项：-fsanitize=address（调试时）
