...
export async function generateMetadata({ params }: PageProps) {
  ...
  return {
    ...
    description:
      typeof task.content === "object" && "objective" in task.content
    ...
    return (
      <div className="space-y-3">
        {content.split("\n").map((paragraph, idx) => (
          <p
            key={idx}
            className={`text-sm text-neutral-300 leading-relaxed ${isCompleted ? "opacity-50 line-through" : ""}`}
          >
            {paragraph}
          </p>
        ))}
    ...
    return (
      <ul className="space-y-2">
        {content.map((item, idx) => (
          <li
            key={idx}
            className={`flex items-start gap-2 text-sm text-neutral-300 ${isCompleted ? "opacity-50 line-through" : ""}`}
          >
    ...
    return (
      <div className="space-y-4">
          ...
          <div>
            ...
            <ul className="space-y-1">
              {taskContent.key_points.map((api, idx) => (
                <li
                  key={idx}
                  className={`flex items-start gap-2 text-xs text-neutral-400 ${isCompleted ? "opacity-50 line-through" : ""}`}
                >
                  <span className="text-neutral-600 font-mono mt-0.5 flex-shrink-0">·</span>
        ...
        {taskContent.practice && (
          <div
            className={`relative border-l-4 border-emerald-500 bg-gradient-to-r from-emerald-500/10 to-transparent pl-4 pr-3 py-3 rounded-r-lg ${isCompleted ? "opacity-50" : ""}`}
          >
      ...
      <header className="bg-neutral-900 border-b border-neutral-800">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 py-4">
          ...
          <div className="flex items-center gap-3 mb-2">
            <span
              className={`inline-block font-mono text-[10px] px-2 py-0.5 rounded border ${colors.bg} ${colors.border} ${colors.text}`}
            >
          ...
          {task.summary && <p className="text-sm text-neutral-400 mt-1">{task.summary}</p>}
        ...
        <div className="max-w-4xl mx-auto px-6 py-6">
            ...
            {prevTask ? (
              ...
              >
                ...
                <span className="text-xs text-neutral-500">
                  (
                  {prevTask.title.length > 20
                    ? prevTask.title.slice(0, 20) + "..."
                    : prevTask.title}
                  )
                </span>
              </Link>
            ...
            {nextTask ? (
              ...
              >
                ...
                <span className="text-xs text-green-500/70">
                  (
                  {nextTask.title.length > 20
                    ? nextTask.title.slice(0, 20) + "..."
                    : nextTask.title}
                  )
                </span>
              </Link>
          ...
          <div className="mt-6 pt-6 border-t border-neutral-800">
            <div className="flex justify-between font-mono text-[10px] text-neutral-500 mb-2">
              ...
              <span>
                {currentIndex + 1}/{sortedTasks.length} 天
              </span>
            </div>
